...
class XliffSdl extends Xliff12
{
    ...
    /**
     * Prepare target statuses attribute.
     *
     * @param array<string, mixed> $segment Segment data
     *
     * @return string Status attribute string
     */
    protected function prepareTargetStatuses(array $segment): string
    {
        $statusMap = [
            'NEW' => '',
            'DRAFT' => 'Draft',
    ...
    protected function rebuildMarks(array $seg, string $translation): string
    {
        ...
        if ($seg['mrk_id'] !== null && $seg['mrk_id'] != '') {
            ...
            $translation = "<mrk mid=\"" . $seg['mrk_id'] . "\" mtype=\"seg\">" . $seg['mrk_prev_tags'] . rtrim(
                    $translation
                ) . $seg['mrk_succ_tags'] . "</mrk>" . $trailingSpaces;
        }
